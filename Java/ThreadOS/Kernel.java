/*/////////////////////////////////////
Edited by: John Neigel, Parker Amundsen
Last Edit: 03/14/2020
Description: The Kernel is essentially the switchboard of the ThreadOS program.
When Syslib recieves instructions from a driver, it passes them here, and the Kernel
interprets those instructions to make the proper calls to each class. Kernel filters
operations using switch statements and constants to identify the operations being requested.
Kernel coordinates thread operations as well as file system operations.
 */////////////////////////////////////
            ...
            case INTERRUPT_SOFTWARE: // System calls
                switch (cmd) {
                    ...
                    case OPEN:    // to be implemented in project
                        if ((myTcb = scheduler.getMyTcb()) != null) {
                            String[] convertedArgs = (String[]) args;
                            return myTcb.getFd(fs.open(convertedArgs[0], convertedArgs[1]));
                        } else return ERROR;
                    case CLOSE:   // to be implemented in project
                        if ((myTcb = scheduler.getMyTcb()) != null) {
                        FileTableEntry ftEnt = myTcb.getFtEnt(param);
                        if (ftEnt == null || fs.close(ftEnt) == false) return ERROR;
                        if (myTcb.returnFd(param) != ftEnt) return ERROR;
                        return OK;
                    }
                        return ERROR;
                    case SIZE:    // to be implemented in project
                        if ((myTcb = scheduler.getMyTcb()) != null) {
                            FileTableEntry ftEnt = myTcb.getFtEnt(param);
                            if (ftEnt != null) return Kernel.fs.fsize(ftEnt);
                        }
                        return ERROR;
                    ...
                    case SEEK:    // to be implemented in project
                    {
                        ...
                        if ((myTcb) != null) {
                            FileTableEntry ftEnt = myTcb.getFtEnt(param);
                            if (ftEnt != null) {
                                int[] offAndWhence = (int[])args;
                                return Kernel.fs.seek(ftEnt, offAndWhence[0], offAndWhence[1]);
                            }
                        }
                        return ERROR;
                    }
                    case FORMAT:  // to be implemented in project
                        return (Kernel.fs.format(param)) ? OK:ERROR;
                    case DELETE:  // to be implemented in project
                        return (Kernel.fs.delete((String) args)) ? OK:ERROR;
                }
                return ERROR;
